...
class Symbolic_Function:
    ...
    @staticmethod
    def from_model(
        model: Union[SBML_Model, BMA_Model], variable: str, seed: int | None = None
    ) -> Symbolic_Function:
    ...
    @staticmethod
    def from_sbml(
        model: SBML_Model, variable: str, seed: int | None = None
    ) -> Symbolic_Function:
        ...
        (bdd_vars, booleans, integers) = build_symbolic_context(
            relevant_variables, seed
        )

        implicant_bdds = [
            bdd_vars.mk_const(False) for _ in range(relevant_variables[variable] + 1)
        ]
        if variable not in model.functions:
            # This variable is a constant. This means change of value is never possible.
            return Symbolic_Function(
                bdd_vars, variable, booleans, integers, implicant_bdds, regulators
            )
        ...
        implicant_bdds[sbml_function.default_result] = implicant_bdds[
            sbml_function.default_result
        ].l_or(default_term)
    ...
    @staticmethod
    def from_bma(
        model: BMA_Model, variable, seed: int | None = None
    ) -> Symbolic_Function:
        regulators = model.functions[variable].inputs.copy()
        relevant_variables = {input: model.variables[input] for input in regulators}
        relevant_variables[variable] = model.variables[variable]
...
class Symbolic_Model:
    ...
    def from_sbml(model: SBML_Model) -> Symbolic_Model:
        ...
        for variable in model.variables:
            implicant_bdds = [
                bdd_vars.mk_const(False) for _ in range(levels[variable] + 1)
            ]
            if variable not in model.functions:
                # This variable is a constant. This means change of value is never possible.
        ...
        return Symbolic_Model(
            bdd_vars, booleans, integers, levels, functions, function_inputs
        )
        ...
        for var in self.functions:
            ...
            if any([bdd.is_true() for bdd in bdds]):
                ...
                function = SBML_Function(
                    inputs=[], output=var, default_result=default_level, terms=[]
                )
            ...
            for result_level, bdd in level_bdds:
                ...
                for bdd in expand_universal_integers(
                    self.ctx, self.integers, self.function_inputs[var], bdd
                ):
                    for clause in bdd.clause_iterator():
                        ...
                        for bdd_var, value in clause.items():
                            if bdd_var in boolean_bdd_variables:
                                ...
                                inputs.add(name)
                                literals.append(
                                    SBML_Proposition(name, CmpOp.EQ, int(value))
                                )
                            else:
                                ...
                                if value:
                                    m = re.match(
                                        "p(.+?)_b(\\d+)",
                                        self.ctx.get_variable_name(bdd_var),
                                    )
                                    ...
                                    inputs.add(name)
                                    literals.append(
                                        SBML_Proposition(name, CmpOp.EQ, value_int)
                                    )
                        disjunction.append(SBML_Expression(LogicOp.AND, literals))
                if len(disjunction) > 0:
                    term = SBML_Term(
                        result_level, SBML_Expression(LogicOp.OR, disjunction)
                    )
                    terms.append(term)
...
def build_symbolic_context(
    levels: Dict[str, int], seed: int | None = None
) -> Tuple[BddVariableSet, Dict[str, BddVariable], Dict[str, List[BddVariable]]]:
    """
    ...
    for var in variables:
        ...
        if max_level == 1:
            ...
            integers[var] = bdd_vars_builder.add_all(
                [f"p{var}_b{x}" for x in range(max_level + 1)]
            )
