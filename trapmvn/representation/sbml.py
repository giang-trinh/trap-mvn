...
class SBML_Proposition:
    ...
    def evaluate_symbolic(
        self, ctx: BddVariableSet, symbolic: Union[BddVariable, List[BddVariable]]
    ) -> Bdd:
        """
        ...
        if type(symbolic) == BddVariable:
            assert (
                self.operator == CmpOp.EQ or self.operator == CmpOp.NEQ
            ), f"Boolean variables only support eq/neq propositions for now."

            positive = False
            if (self.operator == CmpOp.EQ and self.constant == 1) or (
                self.operator == CmpOp.NEQ and self.constant == 0
            ):
                positive = True
        ...
        if type(symbolic) == list:
            ...
            assert (
                0 <= self.constant and self.constant <= max_level
...
class SBML_Term:
    ...
    def __init__(
        self, result: int, expression: Union[SBML_Expression, SBML_Proposition]
    ):
        self.result = result
    ...
    def to_xml(self) -> XmlElement:
        e = XmlElement(
            f"{{{NS_SBML_QUAL}}}functionTerm",
            attrib={f"{{{NS_SBML_QUAL}}}resultLevel": str(self.result)},
        )
        ...
        math = XmlElement(f"math", attrib={"xmlns": NS_MATH})
...
class SBML_Function:
    ...
    def from_xml(element: XmlElement) -> SBML_Function:
        ...
        assert (
            output_tag.attrib[f"{{{NS_SBML_QUAL}}}transitionEffect"]
            == "assignmentLevel"
        ...
        if list_of_inputs:
            for input_tag in list_of_inputs:
                assert (
                    input_tag.attrib[f"{{{NS_SBML_QUAL}}}transitionEffect"] == "none"
                ), f"Unsupported input transition effect in transition {transition_id}."
                inputs.append(
                    str(input_tag.attrib[f"{{{NS_SBML_QUAL}}}qualitativeSpecies"])
                )
        ...
        terms = [
            SBML_Term.from_xml(term)
            for term in list_of_terms.findall(f"{{{NS_SBML_QUAL}}}functionTerm")
        ]
    ...
    def to_xml(self) -> XmlElement:
        ...
        transition = XmlElement(
            f"{{{NS_SBML_QUAL}}}transition",
            attrib={f"{{{NS_SBML_QUAL}}}id": f"tr_{self.output}"},
        )
...
class SBML_Model:
    ...
    def from_xml(tree: XmlElement) -> SBML_Model:
        ...
        functions = {}
        for transition_tag in list_of_transitions.findall(
            f"{{{NS_SBML_QUAL}}}transition"
        ):
