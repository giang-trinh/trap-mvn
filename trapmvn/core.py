...
def _normalize_model(
    model: Union[Petri_Net, Symbolic_Model, SBML_Model, BMA_Model], semantics: str
) -> Petri_Net:
    """
...
def _optimized_translation(
    model: Union[SBML_Model, BMA_Model], unitary: bool
) -> Petri_Net:
    """
    ...
    for variable in model.variables:
        ...
        arity = (
            len(model.functions[variable].inputs) if variable in model.functions else 0
        )
        for i in range(1, arity):
            candidate_encodings.append(
                Symbolic_Function.from_model(model, variable, seed=i)
            )
        candidate_networks = [
            Petri_Net.for_function(f, unitary) for f in candidate_encodings
        ]
        candidate_networks = sorted(
            candidate_networks, key=lambda pn: pn.count_implicants(variable)
        )
...
def _decode_result_mirror(result: Model, petri_net: Petri_Net) -> Dict[str, List[int]]:
    ...
    space: Dict[str, List[int]] = {
        var: list(range(len(places))) for var, places in petri_net.place_map.items()
    }
    for atom in result.symbols(atoms=True):
        atom_str = str(atom)
        (variable, level) = petri_net.places[atom_str]
...
def _encode_asp_problem(
    petri_net: Petri_Net, problem: str = "min", fixed_point_method: str = "deadlock"
) -> Control:
    assert problem in ["min", "max", "fix"], f"Unknown problem type: {problem}."
    assert fixed_point_method in [
        "siphon",
        "deadlock",
    ], f"Unknown fixed-point algorithm: {fixed_point_method}."
    ...
    if problem == "fix" and fixed_point_method == "deadlock":
        ...
        for var in variables:
            ...
            for (s_level, _t_level), implicant_list in petri_net.implicants[
                var
            ].items():
